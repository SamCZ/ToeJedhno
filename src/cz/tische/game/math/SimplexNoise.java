...
public class SimplexNoise {  // Simplex noise in 2D, 3D and 4D
  ...
  public static float BlurredNoise( float stepSize, float x, float y) {
	...
	//X, Y is [0..1]
	public static double SeamlessNoise( double x, double y, double dx, double dy, double xyOffset ) {
		double s = x;
		double t = y;

		double nx = xyOffset + Math.cos(s * 2.0f * Math.PI) * dx / (2.0f * Math.PI);
		double ny = xyOffset + Math.cos(t * 2.0f * Math.PI) * dy / (2.0f * Math.PI);
		double nz = xyOffset + Math.sin(s * 2.0f * Math.PI) * dx / (2.0f * Math.PI);
		double nw = xyOffset + Math.sin(t * 2.0f * Math.PI) * dy / (2.0f * Math.PI);
